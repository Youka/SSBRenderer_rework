...
namespace SSB{
	...
	inline std::vector<GUtils::PathSegment> points_to_path(const Points* points, double size){
	...
	inline std::vector<GUtils::PathSegment> path_to_path(const Path* path){
		std::vector<GUtils::PathSegment> result;
       ...
       inline void get_2d_scale(unsigned src_width, unsigned src_height, unsigned dst_width, unsigned dst_height, double& scale_x, double& scale_y){
        ...
        inline Point get_auto_pos(Align::Position align,
